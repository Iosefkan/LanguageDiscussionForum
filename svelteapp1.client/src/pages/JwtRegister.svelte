<script lang="ts">
    ...
    async function submitForm() {
        ...
        if (valid) {
            ...
            const response = await fetch("/fallback/auth/register", {
                method: "POST",
                headers: {
            ...
            if (result.ok) message = "Success, please confirm your email.";
          ...
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form class="space-y-4 md:space-y-6" on:submit|preventDefault={submitForm}>
                <div>
                  ...
                  <button type="submit" class="w-full text-gray-900 dark:text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Create an account</button>
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">{message}</p>
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                      Already have an account? <a href="#/" on:click={handleLogin} class="font-medium text-primary-600 hover:underline dark:text-primary-500">Login here</a>
                  </p>
              </form>
          </div>
      </div>
